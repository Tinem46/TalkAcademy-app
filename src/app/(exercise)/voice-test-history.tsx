...
export default function VoiceTestHistoryScreen() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      if (response && response.data) {
        ...
        console.log('📚 Voice test history loaded:', (response.data as unknown as VoiceTestHistoryItem[]).length, 'items');
      } else {
  ...
  const playAudio = useCallback(async (audioUrl: string, createdAt: string) => {
    try {
  ...
  const renderHistoryItem = ({ item, index }: { item: VoiceTestHistoryItem; index: number }) => (
    <View style={styles.historyItem}>
      <View style={styles.itemHeader}>
        <View style={styles.scoreContainer}>
          <Text style={[styles.scoreText, { color: getScoreColor(item.voiceScore) }]}>
            {item.voiceScore}
          </Text>
          <Text style={styles.scoreLabel}>{getScoreLabel(item.voiceScore)}</Text>
        </View>
      ...
      <View style={styles.metricsContainer}>
        ...
        <View style={styles.metricItem}>
          ...
          <Text style={styles.metricValue}>{(item.cerRatio * 100).toFixed(1)}%</Text>
        </View>
        <View style={styles.metricItem}>
          <Text style={styles.metricLabel}>Tỷ lệ tạm dừng</Text>
          <Text style={styles.metricValue}>{(item.pauseRatio * 100).toFixed(1)}%</Text>
        </View>
        <View style={styles.metricItem}>
          <Text style={styles.metricLabel}>Độ chính xác</Text>
          <Text style={styles.metricValue}>{(item.finalConsonantAccuracy * 100).toFixed(1)}%</Text>
        </View>
      </View>
      ...
      {item.audioUrl ? (
        ...
        >
          <Ionicons 
            name={playingAudio === item.createdAt ? "pause-circle" : "play-circle-outline"} 
        ...
        <View style={styles.noAudioContainer}>
          <Ionicons name="volume-mute-outline" size={responsiveSize(16)} color="#999" />
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      <MascotWithBubble
        ...
        mascotPosition={{ left: responsiveSize(-20), bottom: responsiveSize(40) }}
        bubblePosition={{ left: responsiveSize(90), top: responsiveSize(80) }}
        bubbleStyle={{ height: responsiveSize(100), width: responsiveSize(200) }}
    ...
    <View style={styles.container}>
      ...
      {error ? (
        <View style={styles.errorContainer}>
          <Ionicons name="alert-circle-outline" size={responsiveSize(48)} color="#F44336" />
